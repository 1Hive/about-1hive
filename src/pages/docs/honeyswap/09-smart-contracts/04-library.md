...
Calculates the address for a pair without making any external calls (see <Link to='/docs/honeyswap/technical-considerations/pair-addresses'>Pair Addresses</Link>).
...
Calls <Link to='/docs/honeyswap/smart-contracts/pair#getreserves'>getReserves</Link> on the pair for the passed tokens, and returns the results sorted in the order that the parameters were passed in.
...
- Useful for calculating optimal token amounts before calling <Link to='/docs/honeyswap/smart-contracts/pair#mint-1'>mint</Link>.
...
- Useful for calculating optimal token amounts before calling <Link to='/docs/honeyswap/smart-contracts/pair#swap-1'>swap</Link>.
