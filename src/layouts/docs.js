...
const Docs = props => {
  ...
  `)
  const isDao = props.path.slice(0, 8) === '/docs/dao'
  ...
  return (
    <Layout path={props.location.pathname} isDocs={true}>
      ...
      <StyledDocs id="docs-header">
        {!isMobile && (isDao ? <SidebarV1 parent={'/docs/'} {...props} /> : <SidebarV2 parent={'/docs/'} {...props} />)}
        <StyledMDX>
          <StyledPageTitle>
            <small style={{ marginBottom: '.5rem' }}>
              {data.allMdx.edges
                .filter(({ node }) => {
                  return node.fields.slug === props.path && node.fields.slug !== '/docs/honeyswap/'
                })
                .map(({ node }) => {
              ...
              return (
                <StyledDocsNavWrapper key={node.id}>
                  <StyledDocsNav>
                    ...
                    {previous &&
                      node.fields.slug !== '/docs/honeyswap/' &&
                  ...
                  <StyledDocsNav>
                    ...
                    {next && next.fields.slug !== '/docs/honeyswap/' && next.fields.parentDir === node.fields.parentDir && (
                    ...
                    {node.fields.slug === '/docs/honeyswap/' && (
                      <StyledLink style={{ alignItems: 'flex-start' }} to={'/docs/honeyswap/protocol-overview/'} rel="next">
                        <small>Next</small>
