...
const SideBar = props => {
  const data = useStaticQuery(graphql`
    query {
      ...
      topNavDocsV2: allDirectory(
        filter: { sourceInstanceName: { eq: "docs" }, relativeDirectory: { eq: "honeyswap" } }
        sort: { fields: name, order: ASC }
      ) {
        edges {
          node {
      ...
      docsV2: allMdx(
        filter: { fileAbsolutePath: { regex: "/docs/honeyswap/" } }
        sort: { order: ASC, fields: fileAbsolutePath }
      ) {
        edges {
          node {
      ...
      v2Reference: allFile(filter: { absolutePath: { regex: "/docs/honeyswap/" }, ext: { eq: ".js" } }) {
        edges {
          node {
            name
            absolutePath
  ...
  const v2Toggle = props.path.slice(0, 15) === '/docs/honeyswap'
